...
(def ^:private operator {:actor-id "op-1" :actor-role :glazing-quality-engineer :phase 3})
...
(defn run-demo! []
  (let [db (store/seed-db) actor (op/build db)]
    (exec! actor "t1" {:op :glass-panel-batch/intake :subject "batch-1"
                       :effect :propose :patch {:id "batch-1" :batch-name "Sakura Float-Line WS-04"}})
    (exec! actor "t2" {:op :glazing-standard-rules/verify :subject "batch-1" :effect :propose})
    (approve! actor "t2")
    (exec! actor "t3" {:op :end-of-line-quality/screen :subject "batch-1" :effect :propose})
    (approve! actor "t3")
    (exec! actor "t4" {:op :actuation/ship-glass-panel-batch :subject "batch-1" :effect :propose})
    (approve! actor "t4")
    (exec! actor "t5" {:op :glass-panel-batch/intake :subject "batch-999"
                       :effect :propose :patch {:id "batch-999" :batch-name "Unregistered"}})
    db))
...
(def ^:private gate-rows
  ["        <tr><td><code>:glass-panel-batch/intake</code></td><td><span class=\"ok\">auto-commit when clean</span></td></tr>"
   "        <tr><td><code>:glazing-standard-rules/verify</code></td><td><span class=\"warn\">ALWAYS human approval</span></td></tr>"
   "        <tr><td><code>:end-of-line-quality/screen</code></td><td><span class=\"warn\">ALWAYS human approval</span></td></tr>"
   "        <tr><td><code>:robotics/simulate-flexural-strength-test</code></td><td><span class=\"warn\">ALWAYS human approval (physics test)</span></td></tr>"
   "        <tr><td><code>:actuation/ship-glass-panel-batch</code></td><td><span class=\"warn\">ALWAYS human approval (actuation)</span></td></tr>"
   "        <tr><td><code>:actuation/issue-glazing-certificate</code></td><td><span class=\"warn\">ALWAYS human approval (actuation)</span></td></tr>"])
(defn render [db]
  (let [ledger (vec (store/ledger db))
        batches (->> (store/all-batches db) (sort-by :id))
        brow (fn [b] (format "        <tr><td>%s</td><td>%s</td><td>%s</td></tr>" (esc (:id b)) (esc (or (:batch-name b) "-")) (status-cell ledger (:id b))))
        brows (str/join "\n" (map brow batches))
        lrows (str/join "\n" (map ledger-row ledger))]
    (str "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-2310</title>"
     "<style>body{font:14px/1.5 sans-serif;margin:0;color:#1a1a1a;background:#f5f5f5}"
     ".bar{background:#0a1a3a;color:#fff;padding:1.2rem 2rem}.bar h1{margin:0;font-size:1.15rem}"
     ...
     "<header class=\"bar\"><h1>Glass manufacturing ops (ISIC 2310) — <code>glassworks</code></h1></header><main>"
     "<section class=\"card\"><h2>Production batches</h2>"
     "<p class=\"muted\">Demo from <code>glassworks.store</code> via <code>glassworks.render-html</code>. No invented data.</p>"
     "<table><thead><tr><th>Batch</th><th>Name</th><th>Last op</th></tr></thead><tbody>" brows "</tbody></table></section>"
