(ns glassworks.operation
  "OperationActor -- one glass-and-glass-products-manufacturing
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (Glass Advisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Tempering
  ...
  One graph run = one glass-and-glass-products-manufacturing operation
  ...
  decision to a human operator (the glazing quality engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/ship-glass-panel-batch`/`:actuation/issue-
  glazing-certificate` ALWAYS reach this node when the governor is
  clean -- see `glassworks.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [glassworks.glassworksadvisor :as glassworksadvisor]
            [glassworks.governor :as governor]
...
(defn build
  ...
  `glassworks.store/Store`).
  opts:
    :advisor      -- a `glassworks.glassworksadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (glassworksadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Glass Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (glassworksadvisor/-advise advisor store request)]
            {:proposal p :audit [(glassworksadvisor/trace request p)]})))

      ;; Tempering Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
